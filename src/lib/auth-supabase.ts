...
export async function getSupabaseUser(request: NextRequest) {
  try {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      ...
      // Extract token from cookie - try multiple patterns
      let tokenMatch = cookieHeader.match(/sb-maichqozswcomegcsaqg-auth-token=([^;]+)/)

      // Fallback: try any sb-*-auth-token pattern (Supabase project ID might differ)
      if (!tokenMatch) {
        tokenMatch = cookieHeader.match(/sb-[^-]+-auth-token=([^;]+)/)
      }
