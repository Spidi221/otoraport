 ...
 */
export function createError(errorType: keyof typeof ErrorTypes, details?: any, context?: Record<string, any>): OtoraportError {
  const type = ErrorTypes[errorType]
  return new OtoraportError(type.code, type.message, type.userMessage, type.statusCode, details, context)
}

/**
 * Log error with proper context
 */
export function logError(error: Error | OtoraportError, context?: Record<string, any>) {
  const timestamp = new Date().toISOString()
  
  if (error instanceof OtoraportError) {
    console.error(`[${timestamp}] OtoraportError:`, {
      code: error.code,
 ...
 */
export function errorToResponse(error: Error | OtoraportError, fallbackMessage?: string) {
  if (error instanceof OtoraportError) {
    return {
      success: false,
